...
SocketcanAdapter::SocketcanAdapter(
  ...
  const std::chrono::duration<float> & receive_timeout_s)
...
std::optional<SocketcanAdapter::socket_error_string_t> SocketcanAdapter::setJoinOverwrite(
  const bool & join)
{
  join_ = join;
  return sendJoin();
...
std::optional<SocketcanAdapter::socket_error_string_t> SocketcanAdapter::receive(
  CanFrame & polymath_can_frame)
{
  struct pollfd fds[1];
  ...
  int return_value = poll(
    fds, NUM_SOCKETS_IN_ADAPTER,
    std::chrono::duration_cast<std::chrono::milliseconds>(receive_timeout_s_).count());

  socket_error_string_t error_string;

  if (return_value < 0) {
    socket_error_string_t err_string = socket_error_string_t("poll failed with error") + strerror(
      errno);
  ...
  if (fds[0].revents & POLLERR) {
    ...
    if (getsockopt(socket_file_descriptor_, SOL_SOCKET, SO_ERROR, &error, &len) < 0) {
      ...
      error_string += socket_error_string_t("Socket error: ") + socket_error_string_t(
        strerror(
          error) +
  ...
  const CanFrame & frame)
{
  ...
  const auto bytes_sent = ::send(
    socket_file_descriptor_, &raw_frame, sizeof(struct can_frame),
    flags);

  return (bytes_sent < 0) ?
         std::optional<socket_error_string_t>(
...
std::optional<SocketcanAdapter::socket_error_string_t> SocketcanAdapter::sendFilters()
{
  ...
  {
    error_output += socket_error_string_t("Failed to send CAN filters: ") + socket_error_string_t(
      strerror(
        errno));
  }
...
std::optional<SocketcanAdapter::socket_error_string_t> SocketcanAdapter::sendErrorMask()
{
  ...
  {
    error_output += socket_error_string_t("Failed to send Error Mask: ") + socket_error_string_t(
      strerror(
        errno));
  }
...
std::optional<SocketcanAdapter::socket_error_string_t> SocketcanAdapter::sendJoin()
{
  ...
  if (0 != setsockopt(
      ...
      sizeof(int32_t)))
  {
    error_output += socket_error_string_t("Failed to set Join Filter ") + socket_error_string_t(
      strerror(
        errno));
  }
...
bool SocketcanAdapter::setOnErrorCallback(
